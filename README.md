...
**Important**: The development environment by default is setup to use the CPU, to be agnostic. The precise development environment needed for GPU training and inference depends on the GPU accelerator being used. The specific version of torch downloaded by uv may differ due to requirements for the GPU drivers.
...
Then from [here](https://figshare.com/articles/figure/wheat_zip/22992317/2), download the following 2 files: `wheat.zip`, `wheat.xml`. You will need to download these both and place them inside `GrainSetData` directory. Finally, do:
...
You may need to give the service around 30 seconds to start up. To test with a sample payload, run
